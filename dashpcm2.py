...
# ─── DATA ─────────────────────────────────────────────────────────────────────
DATA_PATH = os.path.join(os.path.dirname(__file__), "processed_agentic_traces_FIXED_1.csv")
...
def load_data():
    ...
    # ĐỊNH NGHĨA "FAILED" (v2, sau khi sửa lỗi đo has_error):
    # error_share==1.0 (bản cũ) không còn dùng được — sau khi has_error được tính đúng theo
    # từng turn (thay vì bị nhiễm chéo từ lịch sử tích lũy), tỷ lệ lỗi thật rất thấp
    # (TB 1.6% Sonnet, 16-18% Minimax/DeepSeek) nên gần như không session nào đạt đúng 100% lỗi
    # nữa -> ngưỡng cũ sẽ cho failed=0% ở mọi model, mất khả năng phân loại.
    # Dùng "chạm trần ngân sách turn" (n_turns>=50) làm proxy thay thế: đã kiểm chứng cho phân
    # tách có ý nghĩa (Sonnet 9.1%, DeepSeek 44.6%, Minimax 38.6%), độc lập hoàn toàn với
    # has_error (không rủi ro dính lại lỗi cũ), và khớp tự nhiên với chính khuyến nghị
    # Circuit Breaker ở §04 (ngăn chặn đúng nhóm session này).
    sess["failed"]        = (sess["n_turns"] >= 50).astype(int)
...
_minimax_failed_cost  = df_swe_s[(df_swe_s["model"]=="minimax-m2.5") & (df_swe_s["failed"]==1)]["total_cost"].sum()
_deepseek_failed_cost = df_swe_s[(df_swe_s["model"]=="deepseek-v3.1") & (df_swe_s["failed"]==1)]["total_cost"].sum()
_mmx_dpk_failed_cost   = _minimax_failed_cost + _deepseek_failed_cost
...
with tab1:
    ...
    with r2c2:
        with get_card():
            ...
            if ms.empty:
                ...
                pls(f4, h=290)
                f4.update_layout(yaxis_tickformat=".0%", yaxis_range=[0, max(0.05, mt4["err_rate"].max()*1.3)], showlegend=False, xaxis_title="")
                st.plotly_chart(f4, key="chart_mt4", use_container_width=True, config={"displayModeBar":False})
                _top_err = mt4.iloc[0]
                st.markdown(f'<div class="cap w">{_top_err["model"]} có tỷ lệ lỗi/turn cao nhất ({_top_err["err_rate"]*100:.1f}%) — cao hơn nhiều lần so với Claude, nhưng không có model nào đạt mức lỗi cực đoan.</div>', unsafe_allow_html=True)
    ...
    st.markdown(f"""<div class="tbox">
        ...
        • <b>Quy mô:</b> Claude Sonnet chiếm {fp(ms[ms['model']=='claude-sonnet-4-6']['pct_cost'].sum()*100 if not ms.empty else 0)} ngân sách và có tỷ lệ hoàn thành trong ngân sách turn cao nhất. Minimax &amp; Deepseek chiếm {fp(ms[ms['model'].isin(['minimax-m2.5','deepseek-v3.1'])]['pct_cost'].sum()*100 if not ms.empty else 0)} chi phí nhưng chạy dài hơn và tỷ lệ chạm trần turn cao hơn nhiều.<br>
        • <b>Bất thường:</b> Tỷ lệ lỗi/turn cao hơn Claude 10-20 lần và số lượt lặp thường vượt 35 turns ở 2 mô hình rẻ — báo hiệu xu hướng kẹt vòng lặp, được chẩn đoán sâu ở Tab §02.
    </div>""", unsafe_allow_html=True)
...
with tab2:
        ...
        if df_swe_s.empty:
            ...
            if not trap.empty:
                fig_cd1.add_annotation(x=trap['cost_per_turn'].median(), y=trap['n_turns'].max()*0.9,
                    text=f"<b>⚠️ CHẠY DÀI, TỶ LỆ CHẠM TRẦN CAO</b><br>{fc(_mmx_dpk_failed_cost)} chi phí thất bại",
    ...
    with get_card():
        st.markdown('<div class="panel-title">Biểu đồ 2.4: Độ dài phản hồi theo trạng thái lỗi (Output Length vs. Error State)</div>', unsafe_allow_html=True)
        if df_swe_t.empty:
            st.markdown(empty_html(), unsafe_allow_html=True)
        else:
            ...
            # Tính delta cho TỪNG model một cách tổng quát (không giả định trước chiều tăng/giảm) —
            # vì sau khi has_error được sửa đúng, chiều hướng có thể khác nhau giữa các model.
            cap_bits = []
            for m, mlbl in zip(SWE_MODELS, [s.replace("claude-sonnet-4-6","Sonnet").replace("minimax-m2.5","Minimax").replace("deepseek-v3.1","DeepSeek") for s in SWE_MODELS]):
                sub_m = refl[refl["model"]==m]
                states = set(sub_m["has_error"].tolist())
                if states == {0,1}:
                    a = sub_m[sub_m["has_error"]==0]["output_length"].values[0]
                    b = sub_m[sub_m["has_error"]==1]["output_length"].values[0]
                    delta_pct = (b-a)/a*100 if a>0 else 0
                    direction = "tăng" if delta_pct>0 else "giảm"
                    fig_refl.add_annotation(x=m, y=max(a,b), text=f"<b>{delta_pct:+.1f}%</b>",
                        showarrow=True, arrowhead=2, ax=0, ay=-32,
                        font=dict(size=11, color=(CLR_POS if delta_pct>=0 else CLR_NEG), family="Inter"),
                        bgcolor=("rgba(16,185,129,0.12)" if delta_pct>=0 else "rgba(244,63,94,0.12)"),
                        bordercolor=(CLR_POS if delta_pct>=0 else CLR_NEG), borderwidth=1)
                    cap_bits.append(f"{mlbl}: output {direction} {abs(delta_pct):.1f}% khi có lỗi ({a:.0f}→{b:.0f} tokens).")
                elif states:
                    only = list(states)[0]
                    v = sub_m[sub_m["has_error"]==only]["output_length"].values[0]
                    cap_bits.append(f"{mlbl}: không đủ dữ liệu đối chứng (chỉ có trạng thái {'lỗi' if only==1 else 'không lỗi'}, TB {v:.0f} tokens).")
    ...
    with cr2:
        with get_card():
            ...
            _net_productive = swe_budget - _sonnet_failed_cost - _minimax_failed_cost - _deepseek_failed_cost

            fw = go.Figure(go.Waterfall(
                orientation="v",
                measure=["absolute", "relative", "relative", "relative", "total"],
                x=["Ngân Sách SWE-bench", "Lãng phí Sonnet", "Lãng phí Minimax", "Lãng phí DeepSeek", "Chi phí Thực sự Sinh lời"],
                y=[swe_budget, -_sonnet_failed_cost, -_minimax_failed_cost, -_deepseek_failed_cost, None],
                ...
                text=[fc(swe_budget), f"-{fc(_sonnet_failed_cost)}", f"-{fc(_minimax_failed_cost)}", f"-{fc(_deepseek_failed_cost)}", fc(_net_productive)],
            ...
            st.markdown(f'<div class="cap w">{fc(_net_productive)} ({(_net_productive/swe_budget*100) if swe_budget>0 else 0:.1f}%) trong {fc(swe_budget)} thuộc về các session hoàn thành trong ngân sách turn; phần còn lại ({fc(swe_budget-_net_productive)}) rơi vào các session chạm trần 50 turns — dàn trải ở cả 3 model, không riêng Minimax/DeepSeek.</div>', unsafe_allow_html=True)

    st.markdown(f"""<div class="tbox">
        <b>💡 KẾT LUẬN CHẨN ĐOÁN (§02 · SWE-bench, {fi(len(df_swe_s))} sessions):</b><br>
        1. <b>Chi phí tập trung ở 2 model rẻ:</b> Minimax &amp; Deepseek chiếm {fc(wasted_narrow_swe)} tổng chi phí SWE-bench, nhưng đóng góp phần lớn số session chạm trần 50 turns (44.6% DeepSeek, 38.6% Minimax — so với 9.1% ở Sonnet).<br>
        2. <b>Context Bloat:</b> Stack trace nhồi vào context mỗi turn → tokens/turn tăng vọt qua thời gian (Biểu đồ 2.3).<br>
        3. <b>Output theo trạng thái lỗi:</b> Sonnet phản hồi ngắn hơn hẳn khi gặp lỗi (nhiều khả năng là thao tác thử lại nhanh), trong khi Minimax/DeepSeek gần như không đổi độ dài — xem chi tiết ở Biểu đồ 2.4.<br>
        4. <b>Tổng thiệt hại (định nghĩa: session chạm trần 50 turns):</b> {fc(wasted_full_swe)} ({(wasted_full_swe/swe_budget*100) if swe_budget>0 else 0:.1f}% ngân sách SWE-bench), dàn trải ở cả 3 model chứ không riêng Minimax/DeepSeek như nhận định ban đầu.
    </div>""", unsafe_allow_html=True)
...
with tab3:
    ...
    with d1a:
        with get_card():
            ...
            if df_swe_s.empty:
                ...
                st.markdown(f'<div class="cap">Claude Sonnet bão hòa tại Turn {TURN_CUT} ({son_rate_at_cut:.1f}%). Minimax &amp; Deepseek tăng chậm hơn và không bão hòa rõ ràng, đạt lần lượt 61.4%/55.4% khi hết ngân sách 50 turns.</div>', unsafe_allow_html=True)
    ...
    with c36:
        with get_card():
            ...
            try:
                ...
                # Không dùng feature 'model': failed (n_turns>=50) vẫn lệch khá mạnh theo model
                # (Sonnet 9.1%, DeepSeek 44.6%, Minimax 38.6%) nên nếu giữ 'model' làm feature, nó
                # sẽ lấn át phần lớn Feature Importance, che mất tín hiệu hành vi 5-turn đầu thực sự
    ...
    with get_card():
        st.markdown('<div class="panel-title" style="margin-top:0px;">Biểu đồ 3.7 & 3.8: Thuật Toán Học Máy Dự Báo Tổng Chi Phí (SWE-bench)</div>', unsafe_allow_html=True)
        try:
            from sklearn.ensemble import RandomForestRegressor
            if len(ml_data) > 20:
                y_reg = ml_data["total_cost"]
                reg = RandomForestRegressor(n_estimators=100, max_depth=5, random_state=42)
                reg.fit(X, y_reg)

                ml_data = ml_data.copy()
                ml_data["predicted_cost"] = reg.predict(X)

                r1, r2 = st.columns(2)
                with r1:
                    fig_reg = px.scatter(ml_data, x="total_cost", y="predicted_cost", color="resolved",
                    ...
                    max_val = max(ml_data["total_cost"].max(), ml_data["predicted_cost"].max())
    ...
    st.markdown(f"""<div class="tbox">
        ...
        2. <b>Hiệu quả kinh tế:</b> Chặn đứng chi phí thiêu đốt lãng phí, bảo toàn {_pres_pct:.1f}% số session Sonnet hoàn thành trong ngân sách turn ({_son_resolved_at_cut}/{_son_resolved_tot}) — vì từ Turn {TURN_CUT} trở đi, số session hoàn thành thêm tăng rất chậm (plateau).<br>
...
with tab4:
    ...
    # Tính toán ĐỘNG cho KN1 — Circuit Breaker CHỈ áp dụng cho Agent Runner Claude Sonnet
    # (đúng như tên khuyến nghị), nên phạm vi tính toán phải giới hạn ở Sonnet, không gộp
    # cả Minimax/DeepSeek — nếu không, "resolve rate cost" sẽ bị pha loãng/sai lệch vì
    # Minimax/DeepSeek có phân phối n_turns rất khác Sonnet (34-38 turns TB, so với 23 của Sonnet).
    _son_swe_t4 = df_swe_s[df_swe_s["model"] == "claude-sonnet-4-6"]
    _son_t_t4   = df_swe_t[df_swe_t["model"] == "claude-sonnet-4-6"]
    _cost_base = _son_swe_t4["total_cost"].sum()
    _res_rate_base = _son_swe_t4["resolved"].mean() * 100 if not _son_swe_t4.empty else 0.0

    # Chi phí sau khi áp dụng Circuit Breaker tại TURN_CUT (chỉ Sonnet)
    _t_cut = _son_t_t4[_son_t_t4["turn_number"] <= TURN_CUT]
    ...
    # Resolve rate sau khi cắt (chỉ Sonnet, chỉ tính session hoàn thành trong vòng TURN_CUT turns)
    _res_after_cb = _son_swe_t4[(_son_swe_t4["resolved"] == 1) & (_son_swe_t4["n_turns"] <= TURN_CUT)]["session_id"].count()
    _res_rate_after_cb = (_res_after_cb / len(_son_swe_t4) * 100) if not _son_swe_t4.empty else 0.0
    _res_diff = _res_rate_base - _res_rate_after_cb

    kn1, kn2 = st.columns(2)
    with kn1:
        with get_card():
            st.markdown(f'<div class="panel-title">Khuyến nghị 1: Thiết lập tự động ngắt ở bước số {TURN_CUT} (Sonnet)</div>', unsafe_allow_html=True)
    ...
    with kn2:
        with get_card():
            ...
            # 1. Thu hồi từ Circuit Breaker (chỉ Sonnet, đã tính ở KN1)
            # 2. Thu hồi từ Dynamic Routing (chuyển tải Minimax & DeepSeek sang Sonnet)
            w_rout = wasted_narrow_swe
            net_optimal = max(0.0, swe_budget - _saving_cb - w_rout)

            fkn2 = go.Figure(go.Waterfall(
                orientation="v",
                measure=["absolute", "relative", "relative", "total"],
                x=["Ngân Sách Gốc (SWE-bench)", f"Thu Hồi CB Sonnet (≤{TURN_CUT})", "Thu Hồi Routing", "Ngân Sách Tối Ưu"],
                y=[swe_budget, -_saving_cb, -w_rout, None],
                connector=dict(line=dict(color="rgba(14,165,233,0.3)", width=1.5)),
                decreasing=dict(marker=dict(color=CLR_POS)),
                increasing=dict(marker=dict(color=CLR_NEG)),
                totals=dict(marker=dict(color=CLR_BLUE)),
                text=[fc(swe_budget), f"-{fc(_saving_cb)}", f"-{fc(w_rout)}", fc(net_optimal)],
                textposition="outside", textfont=dict(size=11, family="JetBrains Mono"),
                hovertemplate="<b>%{x}</b><br>$%{y:,.2f}<extra></extra>"
            ))
            pls(fkn2, h=300)
            ...
            st.markdown(f'<div class="cap w">Tổng thu hồi tiềm năng: <b>{fc(_saving_cb + w_rout)}</b> ({((_saving_cb + w_rout)/swe_budget*100) if swe_budget>0 else 0:.1f}% ngân sách SWE-bench).</div>', unsafe_allow_html=True)
    ...
    with get_card():
        ...
        st.markdown(f'<div class="cap w">Giảm <b>{fc(_mmx_dpk_failed_cost)}</b> chi phí thất bại từ Minimax/DeepSeek bằng cách định tuyến các task rủi ro cao sang Sonnet — lưu ý: 2 model này vẫn hoàn thành 55-61% session trong ngân sách turn, nên đây là đánh đổi chi phí thấp/độ tin cậy thấp, không phải "100% lãng phí" như đánh giá ban đầu.</div>', unsafe_allow_html=True)
        ...
        <table class="dt">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>Giảm tỷ trọng Minimax & Deepseek ở task rủi ro cao (44.6%/38.6% chạm trần turn); định tuyến sang Sonnet cho các task quan trọng.</td>
                    <td class="r"><b>+{fc(_mmx_dpk_failed_cost)}</b> giảm chi phí thất bại</td>
    ...
    st.markdown(f"""<div class="tbox">
        <b>📋 LỘ TRÌNH TRIỂN KHAI (§04 · dựa trên SWE-bench {fc(swe_budget)}):</b><br>
        1. <b>P0 — Circuit Breaker (Turn {TURN_CUT}, riêng Sonnet):</b> Triển khai ngay lập tức. Tiết kiệm ước tính <b>{fc(_saving_cb)}</b> trên chi phí Sonnet, rủi ro ảnh hưởng năng lực Sonnet &lt;{_res_diff:.1f} điểm %.<br>
        2. <b>P1 — Dynamic Routing:</b> Giảm tải Minimax &amp; Deepseek ở task rủi ro cao. Giảm trực tiếp <b>{fc(_mmx_dpk_failed_cost)}</b> chi phí thất bại.<br>
        3. <b>P2 — Context Pruning:</b> Áp dụng tóm tắt ngữ cảnh tự động khi Token vượt 20K.<br><br>
        👉 <b>Kỳ vọng:</b> Giảm <b>{((_saving_cb + _mmx_dpk_failed_cost)/swe_budget*100) if swe_budget>0 else 0:.1f}% chi phí vận hành</b> trên SWE-bench mà không suy giảm đáng kể tỷ lệ hoàn thành tác vụ.
    </div>""", unsafe_allow_html=True)

# ─── FOOTNOTE ─────────────────────────────────────────────────────────────────
st.markdown(f"""<div class="fnt">
    <b>(1) Định nghĩa "failed":</b> session chạm trần 50 turns (n_turns≥50) — dùng thay cho ngưỡng error_share==1.0 cũ, vì sau khi sửa lỗi đo has_error (lỗi tính trên toàn bộ lịch sử tích lũy thay vì từng turn), tỷ lệ lỗi thật rất thấp nên ngưỡng cũ không còn phân loại được. wasted_narrow (Tab 01, theo bộ lọc) = {fc(wasted_narrow)} (tổng chi Minimax+Deepseek, không phải 100% lãng phí); wasted_full = {fc(wasted_full)} (chi phí các session chạm trần turn).<br>
    <b>(1b) Định nghĩa (Tab 02-04 · cố định SWE-bench):</b> wasted_narrow_swe={fc(wasted_narrow_swe)} (tổng chi Minimax+Deepseek); wasted_full_swe={fc(wasted_full_swe)} (chi phí các session chạm trần turn, cả 3 model) trên tổng {fc(swe_budget)} ngân sách SWE-bench.<br>
    <b>(2) Dữ liệu:</b> processed_agentic_traces_FIXED_1.csv (đã sửa lỗi has_error) · {fi(len(df_sess_raw))} sessions · {fi(len(df_turns_raw))} traces · {fc(df_turns_raw['turn_cost'].sum())} tổng ngân sách gốc.<br>
