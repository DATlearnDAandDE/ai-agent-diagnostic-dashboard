...
with tab2:
    ...
    with cs1:
        with get_card():
            st.markdown('<div class="panel-title">Biểu đồ 2.4: Phân bổ lượng dữ liệu thực tế AI cần xử lý</div>', unsafe_allow_html=True)
            st.markdown('<div class="warn-bar">⚠️ Có Prompt tiêu hao nhiều Token & Chi phí hơn do độ khó của Benchmark</div>', unsafe_allow_html=True)
            ...
            for sv, sl in [(0,"Không prompt"),(1,"Có prompt")]:
                ...
                p_metrics.append({
                    "prompt": sl,
                    "tokens_k": (st_["input_tokens"].mean()/1000) if not st_.empty else 0,
                    "cost_session": ss["total_cost"].mean() if not ss.empty else 0,
                    "err_rate": (st_["has_error"].mean()*100) if not st_.empty else 0,
                    "n_turns": ss["n_turns"].mean() if not ss.empty else 0
                })
            
            if len(p_metrics) == 2:
                df_pm = pd.DataFrame(p_metrics)
                f_cd4 = make_subplots(rows=1, cols=3, subplot_titles=("Tokens/Turn (K)", "Cost/Session ($)", "Error Rate (%)"))
                
                f_cd4.add_trace(go.Bar(
                    x=df_pm["prompt"], y=df_pm["tokens_k"],
                    marker_color=[MUT, "#f43f5e"], text=[f"{v:.1f}K" for v in df_pm["tokens_k"]],
                    textposition="outside", showlegend=False
                ), row=1, col=1)
                
                f_cd4.add_trace(go.Bar(
                    x=df_pm["prompt"], y=df_pm["cost_session"],
                    marker_color=[MUT, "#f43f5e"], text=[f"${v:.2f}" for v in df_pm["cost_session"]],
                    textposition="outside", showlegend=False
                ), row=1, col=2)
                
                f_cd4.add_trace(go.Bar(
                    x=df_pm["prompt"], y=df_pm["err_rate"],
                    marker_color=[MUT, "#f43f5e"], text=[f"{v:.1f}%" for v in df_pm["err_rate"]],
                    textposition="outside", showlegend=False
                ), row=1, col=3)
                
                pls(f_cd4, h=240)
                f_cd4.update_layout(showlegend=False, margin=dict(l=30, r=20, t=36, b=24))
                st.plotly_chart(f_cd4, key="chart_cd4", use_container_width=True, config={"displayModeBar":False})
            st.markdown('<div class="cap">Hiển thị giá trị thực tế: Có prompt phình context gấp 3.5× (xem CD5 để giải mã nguyên nhân).</div>', unsafe_allow_html=True)
    
    with cs2:
        with get_card():
            st.markdown('<div class="panel-title">Biểu đồ 2.5: Phân loại mức độ phức tạp của từng công việc</div>', unsafe_allow_html=True)
            ct = pd.crosstab(df_sess_raw["benchmark"], df_sess_raw["is_system_prompt_present"]).reset_index()
            ct.columns = ["benchmark", "Không Prompt", "Có Prompt"]
            
            f_cd5 = px.bar(
                ct, x="benchmark", y=["Không Prompt", "Có Prompt"],
                barmode="stack", color_discrete_map={"Không Prompt": MUT, "Có Prompt": "#0ea5e9"},
                text_auto=True
            )
            pls(f_cd5, h=240)
            f_cd5.update_layout(
                xaxis_title="Benchmark", yaxis_title="Số Sessions",
                legend=dict(orientation="h", y=1.12, x=0, font=dict(size=10))
            )
            f_cd5.update_traces(textposition="inside", textfont=dict(size=11, family="JetBrains Mono"))
            st.plotly_chart(f_cd5, key="chart_cd5", use_container_width=True, config={"displayModeBar":False})
            st.markdown('<div class="cap w">100% Có Prompt thuộc SWE-bench (task nặng). 100% Không Prompt thuộc GAIA/Wildclaw. Benchmark chính là nguyên nhân gốc!</div>', unsafe_allow_html=True)

    cr1, cr2 = st.columns(2)
    with cr1:
        with get_card():
            st.markdown('<div class="panel-title">Biểu đồ 2.6: Tỷ lệ Tiền sinh lời vs Tiền bị lãng phí</div>', unsafe_allow_html=True)
            if df_s.empty:
                st.markdown(empty_html(), unsafe_allow_html=True)
            else:
    ...
    with cr2:
        with get_card():
            st.markdown('<div class="panel-title">Biểu đồ 2.7: Vì sao trợ lý AI đắt tiền (Opus) lại giúp tiết kiệm hơn?</div>', unsafe_allow_html=True)
            ...
            if pd.notna(_sa) and pd.notna(_oa) and _st > 0 and _ot > 0:
                # Phân rã chuẩn đại số:
                # Delta = Opus_Cost - Sonnet_Cost = (Turns_opus - Turns_sonnet)*CostPerTurn_sonnet + Turns_opus*(CostPerTurn_opus - CostPerTurn_sonnet)
                c_per_turn_son = _sa / _st
                c_per_turn_opu = _oa / _ot
                turn_effect = (_ot - _st) * c_per_turn_son
                rate_effect = _ot * (c_per_turn_opu - c_per_turn_son)
                
                fw = go.Figure(go.Waterfall(
                    orientation="v",
                    measure=["absolute", "relative", "relative", "total"],
                    x=["Sonnet (Baseline)", "Hiệu ứng ít Turns", "Hiệu ứng Đơn giá", "Opus (Total Cost)"],
                    y=[_sa, turn_effect, rate_effect, None],
                    ...
                    text=[fc(_sa), f"{fc(turn_effect)}", f"+{fc(rate_effect)}", fc(_oa)],
            ...
            st.markdown('<div class="cap">Opus dứt điểm nhanh hơn (trung bình ít turns hơn) bù đắp hoàn toàn cho đơn giá token cao hơn.</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with d1b:
        with get_card():
            st.markdown('<div class="panel-title">Biểu đồ 3.2: Chi thêm tiền liệu có mang lại kết quả tốt hơn?</div>', unsafe_allow_html=True)
...
with tab4:
    ...
    # Tính toán ĐỘNG cho KN1 từ tập dữ liệu đang lọc
    _cost_base = df_s["total_cost"].sum()
    _res_rate_base = df_s["resolved"].mean() * 100 if not df_s.empty else 0.0
    
    # Chi phí sau khi áp dụng Circuit Breaker tại TURN_CUT
    _t_cut = df_t[df_t["turn_number"] <= TURN_CUT]
    ...
    _res_after_cb = df_s[(df_s["resolved"] == 1) & (df_s["n_turns"] <= TURN_CUT)]["session_id"].count()
    _res_rate_after_cb = (_res_after_cb / len(df_s) * 100) if not df_s.empty else 0.0
    ...
    with kn2:
        with get_card():
            ...
            # Tính toán động các trụ cột thu hồi lãng phí:
            # 1. Thu hồi từ Circuit Breaker
            # 2. Thu hồi từ Dynamic Routing (loại bỏ minimax & deepseek 100% fail)
            w_rout = df_s[df_s["model"].isin(["minimax-m2.5","deepseek-v3.1"])]["total_cost"].sum()
    ...
    with get_card():
        st.markdown('<div class="panel-title">Khuyến nghị 3: Không dùng AI kém cho các công việc khó</div>', unsafe_allow_html=True)
        if ms.empty:
            st.markdown(empty_html(), unsafe_allow_html=True)
        else:
        ...
        <table class="dt">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>Ngừng cấp quota cho Minimax & Deepseek ở task phức tạp; định tuyến trực tiếp sang Sonnet/Opus.</td>
                    <td class="r"><b>+{fc(wasted_narrow)}</b> thu hồi 100%</td>
    ...
    st.markdown(f"""<div class="tbox">
        ...
        1. <b>P0 — Circuit Breaker (Turn {TURN_CUT}):</b> Triển khai ngay lập tức. Tiết kiệm ước tính <b>{fc(_saving_cb)}</b>, rủi ro ảnh hưởng năng lực &lt;{_res_diff:.1f}%.<br>
        2. <b>P1 — Dynamic Routing:</b> Ngừng quota Minimax &amp; Deepseek. Thu hồi trực tiếp <b>{fc(wasted_narrow)}</b> chi phí thiêu đốt.<br>
        3. <b>P2 — Context Pruning:</b> Áp dụng tóm tắt ngữ cảnh tự động khi Token vượt 20K.<br><br>
        👉 <b>Kỳ vọng:</b> Giảm <b>&gt;{((_saving_cb + wasted_narrow)/_cost_base*100) if _cost_base>0 else 0:.1f}% chi phí vận hành</b> mà không suy giảm tỷ lệ hoàn thành tác vụ.
    </div>""", unsafe_allow_html=True)
